...
func (t *Tree) executeShapedCandidate(ctx context.Context, w work, m engine.Mutant, opts runOptions, runEnv []string) (engine.MutantOutcome, string, bool, error) {
	...
	for _, g := range w.groups {
		...
		// this stepBudget.
		out, groupKiller, groupMemoryDecided, diagnostic, err := seams.runMutantShaped(ctx, scratch, cleanScratch, scratchMutant, g.pkgs, g.runRegex, stepBudget(g, opts), g.flags, env, cleanEnv, opts.bounds)
		if diagnostic != "" {
			if m.Operator == "structural: interface-satisfaction" {
				...
				ran, passed, _, cleanErr := seams.testProbe(ctx, cleanScratch, g.pkgs[0], g.runRegex, opts.OracleTimeout, g.flags, cleanEnv, opts.bounds)
