...
func TestInspectFindingsIncludesFullyAttestedDetachedRecord(t *testing.T) {
	...
	views, _, err := inspectFindings(context.Background(), tree, testStore(t, dir), gomutant.FilterRecords([]gomutant.Finding{finding}, gomutant.RecordFilter{Label: "REQ-A"}), findingFilters{judge: true}, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	views, _, err = inspectFindings(context.Background(), tree, testStore(t, dir), gomutant.FilterRecords([]gomutant.Finding{finding}, gomutant.RecordFilter{Label: "REQ-other"}), findingFilters{judge: true}, nil)
...
func TestInspectFindingsCarriesCandidateEvidence(t *testing.T) {
	...
	views, _, err := inspectFindings(context.Background(), tree, testStore(t, dir), []gomutant.Finding{finding}, findingFilters{judge: true}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
// elapsing under a large delta — carries the reconcile's persisted drop
// in the error text, as the structured face's does: the bound is the
// face's seam, so the exit the bound gates is reachable
// (REQ-mcp-findings-doc, REQ-exec-cancellation).
func TestRunCommandRenderBoundExitCarriesThePersistedDrop(t *testing.T) {
	if testing.Short() {
		t.Skip("runs go test for one mutant")
	}
	t.Setenv("XDG_CACHE_HOME", t.TempDir())
	...
	var output bytes.Buffer
	err := runCommand(context.Background(), runOptions{dir: dir, findingsFile: defaultFindings, budget: 1, jobs: 2, oracleTimeout: 2 * time.Minute, output: &output})
