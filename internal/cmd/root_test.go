...
func TestInspectFindingsIncludesFullyAttestedDetachedRecord(t *testing.T) {
	...
	views, err := inspectFindings(context.Background(), tree, testStore(t, dir), []gomutant.Finding{finding}, "REQ-A", "", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	views, err = inspectFindings(context.Background(), tree, testStore(t, dir), []gomutant.Finding{finding}, "REQ-other", "", "", nil)
...
func TestInspectFindingsCarriesCandidateEvidence(t *testing.T) {
	...
	views, err := inspectFindings(context.Background(), tree, testStore(t, dir), []gomutant.Finding{finding}, "", "", "", nil)
	if err != nil {
		t.Fatal(err)
	}
