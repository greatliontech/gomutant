...
func TestEphemeralRefusesReplacementOutsideOracleLinkedSet(t *testing.T) {
	...
	genSource, err := os.ReadFile(filepath.Join(dir, "genp", "gen.go"))
	if err != nil {
		t.Fatal(err)
	}
	// The mutation edits the on-disk content rather than restating it:
	// a hand-copied literal drifts on the next fixture edit (it already
	// forced one).
	mutatedGen := strings.Replace(string(genSource), "type G struct{}", "type G struct{ X int }", 1)
