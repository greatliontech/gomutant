...
func TestInitFunctionsAreExcludedLoudly(t *testing.T) {
	...
	working := "package lib\n\nvar registry = map[string]int{}\n\nfunc init() { registry[\"a\"] = 1 }\n\nfunc init() { registry[\"b\"] = 2 }\n\nfunc Wired(k string) int { return registry[k] }\n"
	if err := os.WriteFile(filepath.Join(tmp, "lib", "wired.go"), []byte(working), 0o644); err != nil {
		t.Fatal(err)
	}
	generated := "// Code generated by fixture. DO NOT EDIT.\n\npackage lib\n\nfunc init() { registry[\"g\"] = 3 }\n"
	if err := os.WriteFile(filepath.Join(tmp, "lib", "genwired.go"), []byte(generated), 0o644); err != nil {
		t.Fatal(err)
	}
	tr, err := Load(tmp)
	if err != nil {
	...
	// Changed scope, init-only delta: residue names the exclusion, no
	// mislabel as churn, no abort.
	refChangedInit := strings.Replace(working, "registry[\"b\"] = 2", "registry[\"b\"] = 3", 1)
	ref := func(content string) func(string) ([]byte, bool) {
	...
	targets, residue, err := tr.DiscoverChangedContext(context.Background(), []string{"lib/wired.go"}, ref(refChangedInit))
	if err != nil {
		t.Fatal(err)
	}
	if len(targets) != 0 {
	...
	_, residue, err = tr.DiscoverChangedContext(context.Background(), []string{"lib/wired.go"}, ref(working))
	if err != nil {
		t.Fatal(err)
	}
	if len(residue) != 1 || !strings.Contains(residue[0].Reason, "formatting-only churn") {
	...
	// class alone: a generated file's changed init is not an exclusion
	// the caller could act on.
	refGen := strings.Replace(generated, "registry[\"g\"] = 3", "registry[\"g\"] = 4", 1)
	targets, residue, err = tr.DiscoverChangedContext(context.Background(), []string{"lib/genwired.go"}, func(p string) ([]byte, bool) {
		if p == "lib/genwired.go" {
			return []byte(refGen), true
		}
		return nil, false
	})
	if err != nil {
	...
	if len(targets) != 0 {
		t.Fatalf("generated-file delta produced targets: %v", targets)
	}
	if len(residue) != 1 || !strings.Contains(residue[0].Reason, "generated file") {
