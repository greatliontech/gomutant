...
- `analysis_budget_sec` (mcp, cli as `analysis-budget`) — wall-clock bound on each freshness-proof pass (the observed union's proof capture and each measured target's producer validation, each pass entitled to the whole budget; 0, the default on both faces, unbounded): a subject the budget leaves unproven lands an unavailable observation proof naming the budget — the target's measurement stands, and a subject whose reuse rests on the proof re-executes before reuse — and the run continues instead of stalling in the pass; the `proofs` preparation line prices the pass (subjects, packages) before it is paid.
...
- `progress-interval` (cli) — cadence of the cumulative progress line (targets committed and how many of them machine-local, candidates, kills, elapsed); 0 disables.
...
On the CLI the FIRST interrupt (Ctrl-C or SIGTERM) drains
gracefully: in-flight mutants finish under their own oracle budgets,
the drained target's measured prefix commits as an ordinary
candidate-capped record, and re-running the same command measures
only the remainder; a SIGTERM drain is additionally deadline-bounded
(a supervisor's stop banks what fits its kill window, then
hard-cancels cleanly before SIGKILL); a second signal of either kind
cancels hard and discards unfinished work whole.
...
- `changed` (mcp, cli) — cut every record's open survivors against this git ref's delta: the survivors on lines the working tree added since the ref are listed and counted distinctly from the symbol's pre-existing remainder (the number a change's own judgment needs); only a record of a symbol the ref changed, measured against its current body, can be placed — the rest is remainder; loads the tree to place positions, no freshness derived (state stays recorded).
