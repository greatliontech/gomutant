...
| [served-record-provenance-frozen](served-record-provenance-frozen.md) | pure serves carry dirty provenance verbatim, so records born dirty (the attest-before-commit dev loop) never promote to the repo document and their attestations stay in the discardable machine-local overlay - invisible to teammates and CI | cross-tool train chunk 26 (serve-path provenance re-stamp) |
