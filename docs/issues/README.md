...
| [oracle-ephemeral-root-undeclared](oracle-ephemeral-root-undeclared.md) | the tool-minted oracle TMPDIR is never declared at ingest, so every temp-touching oracle is runtime-unverifiable — survivors unbucketed, records non-reusable; one-seam fix via WithEphemeralTempRoot; plus mutant-written tree drift deserves a named reason | cross-tool train chunk 85 |
