...
| [delta-line-survivor-view](delta-line-survivor-view.md) | a changed-ref campaign reports whole-symbol survivors; the delta's added-line survivors need hand reconstruction from the JSON dump and the diff | user decision |
| [ephemeral-test-pkg-shorthand](ephemeral-test-pkg-shorthand.md) | `ephemeral --test-pkg .` refuses the shorthand a module-local caller reaches for | user decision |
| [bracket-path-module-relative-in-workspace](bracket-path-module-relative-in-workspace.md) | a relative bracket path joins onto the target module, so a nested workspace module never matches a root-module surface; absolute paths force machine-local records | user decision |
