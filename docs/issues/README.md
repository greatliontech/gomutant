...
| [mcp-long-running-runs](mcp-long-running-runs.md) | MCP client abandonment is not observable when cancellation is not propagated | cross-tool train chunk 41 |
...
| [fresh-stamp-manifest-base](fresh-stamp-manifest-base.md) | fresh-measure dirtiness resolves observation manifests against the git toplevel and Committable against the one store module dir - in monorepo/workspace layouts runtime-input dirt is invisible and a false-clean portable row can land (re-measure cost, never a wrong verdict) | cross-tool train chunk 31 |
| [growth-drift-promotion-untested](growth-drift-promotion-untested.md) | growth and drift serves recompute provenance but no test pins their dirty-to-clean promotion shape (Dirty/Commit/layer placement) | cross-tool train chunk 31 |
| [rapid-oracle-records-never-plainly-serve](rapid-oracle-records-never-plainly-serve.md) | rapid-oracle findings fail the plain serve match past the scalar guard and re-measure every campaign; plain-oracle records serve fine | cross-tool train chunk 31 |
| [findings-output-unbounded](findings-output-unbounded.md) | a bare findings call returns the whole document (~131k chars in the field); no document bound, no summary view; stale reasons inline unbounded test lists | cross-tool train chunk 30 |
| [changed-mode-oracle-delta-routing](changed-mode-oracle-delta-routing.md) | test-only deltas classify wholly as residue while the stale findings their oracles moved sit unmeasured; the residue row should name the re-measure action | cross-tool train chunk 30 |
| [attest-and-promotion-visibility](attest-and-promotion-visibility.md) | attest succeeds silently with no layer echo, and promoted records leave the findings document uncommitted with no warning - layer state invisible at the moments it matters | cross-tool train chunk 30 |
| [detached-record-lifecycle](detached-record-lifecycle.md) | detached records with attestations never re-attach after renames and nothing says so; no prune or retarget verb, so refactors accrete dead overlay records forever | cross-tool train chunk 31 |
| [plan-and-progress-cosmetics](plan-and-progress-cosmetics.md) | --plan leads with a zeroed summary line that reads like a result; the candidates counter freezes during kill confirmation and briefly reads as a hang | cross-tool train chunk 30 |
