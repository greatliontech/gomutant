...
findings rather than refusing the run. Serial and concurrent selected runs are
already proven, while uncapped whole-tree completion remains to
be demonstrated. Weakening runtime-input identity or
...
Run only in an overnight window from the repository root. The command timeout
bounds work through the findings commit; the oracle timeout independently
bounds each test process:

```
gomutant run --jobs 8 --timeout 10h --oracle-timeout 10m
```
