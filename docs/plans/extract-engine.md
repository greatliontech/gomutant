...
- [ ] 8a. The stipulator seam, gomutant side: Tree.Fresh (the REQ-result-stale pin check as a query, for a caller's freshness reminder) and the stipulator targets-export adapter (REQ-target-producers).
- [ ] 8b. stipulator swap: the targets export on the stipulator side, gomutant dependency, delete the extracted engine from internal/harden + backends/golang, rewire harden/ephemeral/reminder atop gomutant, records adopt the finding document (stipulator spec amendment).
