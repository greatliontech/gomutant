...
- [ ] 9. Agent-first MCP face: server instructions and tool descriptions that teach when to use what; responses restructured for token economy with next-action guidance; run tool streams MCP progress notifications from the engine's progress seam (keep-alive against client tool deadlines, phase-labeled, the stipulator gate treatment) plus long-run advisory (native-Tasks work stays parked on its issue)
