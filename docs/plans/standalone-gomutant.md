...
- [x] 8. Record and report the mutation surface by operator and disposition — generated, discarded, killed, and survived — so users can distinguish oracle strength from absent or inapplicable operators.
- [x] 9. Add standalone run controls for package and symbol filtering, signal-driven cancellation, deterministic progress and final summaries, and explicit cache/force decisions without changing advisory exit semantics.
- [x] 10. Exercise gomutant against its own packages with bounded repeatable targets, strengthen tests for every non-equivalent survivor found, and record only reasoned equivalent-mutant attestations.
