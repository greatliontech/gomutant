...
`,
	...
	inspect := func(vouches ...string) string {
		...
		inspection, err := tree.InspectFinding(ctx, finding, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestRunRecordsPackageProcessDischarges(t *testing.T) {
	...
	attestedInspection, err := crossTree.InspectFinding(ctx, attested[0], nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	crossInspection, err := crossTree.InspectFinding(ctx, cross[0], nil)
	if err != nil {
		t.Fatal(err)
	}
